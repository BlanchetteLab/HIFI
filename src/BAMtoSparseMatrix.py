...
def convert_sam_to_bam(output_prefix,bam_filepath,noPG=False):
    """converts SAM file to BAM, and splits interactions by chr_A and chr_B"""
    #   remove old tmp files from output directory
    for filepath in glob.glob(''.join([output_prefix,"*.tmp"])):
        os.remove(filepath)
    #	view BAM file as SAM using SAMtools
    if noPG:
        sam_cmd = [samtools_exe,"view","--no-PG",bam_filepath]
    else:
        sam_cmd = [samtools_exe,"view",bam_filepath]
    convert_process=subprocess.Popen(sam_cmd,
         stdout=subprocess.PIPE,stderr=subprocess.PIPE)
    #	split interactions by chr_A and chr_B
    split_process=subprocess.Popen(["awk","-F","\t","{ID_A=$1; chr_A=$3; start_A=$4; getline; ID_B=$1; chr_B=$3; start_B=$4; if(ID_A!=ID_B){print \"Error - BAM read pairs are not sequential (Stopping)\"; exit} if(chr_A>chr_B){tmp_chr=chr_A; tmp_start=start_A; chr_A=chr_B; start_A=start_B; chr_B=tmp_chr; start_B=tmp_start} filename=\""+output_prefix+".\"chr_A\"_\"chr_B\".tmp\"; print ID_A,chr_A,start_A,chr_B,start_B > filename}"],
        stdin=convert_process.stdout,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
...
with open(LOG_filepath,"w") as LOG:
    ...
    if(len(glob.glob(''.join([output_prefix,"*.tmp"]))) > 0):
        ...
        #	try with --no-PG flag
        convert_sam_to_bam(output_prefix,args.bam_filepath,noPG=True)
        if(len(glob.glob(''.join([output_prefix,"*.tmp"]))) > 0):
            print("Done",file=sys.stderr)
        else:
            #   problem encountered while parsing BAM file
            print("Error - BAM file was not processed correctly. Please check that SAMtools and awk are installed correctly (and work). See examples of called SAMtools and awk commands below:",file=sys.stderr)
            print('\n'.join([' '.join(["\nSAMtools command (should print the first ten lines of the BAM to stdout):\n\t",samtools_exe,"view","--no-PG",args.bam_filepath,'|',"head"]),' '.join(["\nSAMtools+awk command (should create sparse matrix files):\n\t",samtools_exe,"view","--no-PG",args.bam_filepath,'|',"awk","-F","\'\\t\'","\'{ID_A=$1; chr_A=$3; start_A=$4; getline; ID_B=$1; chr_B=$3; start_B=$4; if(ID_A!=ID_B){print \"Error - BAM read pairs are not sequential (Stopping)\"; exit} if(chr_A>chr_B){tmp_chr=chr_A; tmp_start=start_A; chr_A=chr_B; start_A=start_B; chr_B=tmp_chr; start_B=tmp_start} filename=\""+output_prefix+".\"chr_A\"_\"chr_B\".tmp\"; print ID_A,chr_A,start_A,chr_B,start_B > filename}\'"])]),file=sys.stderr)
            sys.exit(-2)
